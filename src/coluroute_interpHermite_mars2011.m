function Hermite040826 = coluroute_interpHermite_mars2011(pic_init,etalonnage,nomfich)

% toilettage de version oct 2007 pour transfert au CECP

% application de l'étalonnage aux 27 sources
pic=etalonnage(1,:).*pic_init+etalonnage(2,:);
a_moy=mean(etalonnage(1,:));
% affectation de 0 si valeur négative
for i=1:length(pic)
    if pic(i)<0 pic(i)=0
    end
end

% Définition du tableau CIE
...
for colonne=6:7,
    x=[0 0.5 1 2 5.5 12];
    y=tab_cie([1 3 5 9  29],col_remplie(colonne));
   % y(5)=0.01;
